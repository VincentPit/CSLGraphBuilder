...
**Open / next:**
**P14 cross-session semantic memory** — per-user persona summary lives on `User.metadata` (no schema migration, see §5.4); `SemanticMemoryService.load_persona` splices it into the LLM system prompt above SOURCES (the §6.1 sketch's `USER SEMANTIC SUMMARY` slot) on every turn via `QAService`; refresh runs synchronously on `DELETE /qa/sessions/{id}` (captures the just-ended session's turns before delete) and as a background `asyncio.create_task` whenever a new session is created for the user; LLM-driven summariser with a deterministic concat fallback so hermetic tests don't need an LLM.
...
| **P14** | Cross-session semantic memory — per-user persona summary stashed in `User.metadata` (no schema migration); `SemanticMemoryService.load_persona` splices it into the LLM system prompt above SOURCES (§6.1's `USER SEMANTIC SUMMARY` slot); refresh runs synchronously on `DELETE /qa/sessions/{id}` (captures the just-ended session's turns first) and as a background `asyncio.create_task` whenever a new session is created; deterministic concat fallback when no LLM is wired | ✅ shipped | — |
| **+ MutationCard / DebugPane in `/chat`** | UI for §7 + §8 — `MutationCard.tsx` renders mutating tool calls inline alongside assistant bubbles (auto-summarised header per tool, approve/reject inline + deep-link to /curation); `DebugPane.tsx` toggled via `?debug=1` shows request_id, memory trace, tool-calls table, per-claim faithfulness chips. Composer gains opt-in toggles for `enable_tools` / `enable_mutations` persisted to localStorage. `AskResponse` extended with `tool_calls` + `faithfulness`; new API wrappers for `/qa/proposals/{id}/apply` + `/reject`. | ✅ shipped | — |
| **+ Streaming × tool-use combo** | `/qa/ask/stream` honours `enable_tools` + `enable_mutations` (FOLLOWUPS §3, Option A). Agentic loop runs to completion before the answer streams; tool activity surfaces as `tool_call` events between `phase("tools")` and `phase("generating")`, and the `done` event carries `tool_calls`. Final answer arrives as a single `delta` rather than mid-loop interleaving — provider-portable. | ✅ shipped | — |
...
- **Cross-session persona — two triggers, not one (P14).** §5.4's prose said "after a session ends, an LLM produces a persona/preferences summary" but the codebase has no explicit "session end" event (clients just go idle). Shipped with two complementary triggers: (a) `DELETE /qa/sessions/{id}` runs `refresh_persona(force=True, include_session_ids=[...])` *synchronously* before the actual delete, so the just-ended turns flow into the persona — a one-time cost on an explicit user action; (b) every new-session creation in `QAService._resolve_session` fires-and-forgets `asyncio.create_task(refresh_persona(user_id))` so chats that drift idle without an explicit delete still get summarised on the next visit. The freshness gate (`min_new_sessions_to_refresh` + `META_COVERS` count) means the background path is a fast no-op when nothing has changed since the cached summary. Persona storage lives on `User.metadata` (no schema migration — `user_models.py:36` already flagged the metadata bag as the intended P14 hook). The summariser carries the existing persona forward in the prompt rather than re-synthesising from scratch each time, so longitudinal context survives across refreshes. Persona text is spliced into the LLM **system prompt** (not the user prompt) above SOURCES with a `USER SEMANTIC SUMMARY (cross-session, for tone/depth only — never cite)` header — that wording matters because the persona is *not* grounded evidence and we don't want the model citing it as a source. The service degrades gracefully on every failure path: anonymous traffic → empty persona, no LLM → deterministic concat fallback, embedding model unavailable → empty `semantic_embedding` (the text-only persona still works for system-prompt splicing; the embedding is reserved for future cross-session vector recall).
- **Chunk hydration was silently broken — `DocumentChunk(id=...)` (live-run bug, 2026-05-11).** `Neo4jDocumentRepository._create_chunk_from_data` passed `id=` to the `DocumentChunk` dataclass constructor, but `DocumentChunk` is a `@dataclass` over an ABC base whose generated `__init__` doesn't accept `id` (the id is auto-set inside `DomainEntity.__init__` via `__post_init__`). Every `get_chunks_by_ids` / `get_chunk_with_neighbours` call raised `TypeError`, which was caught and logged at `DEBUG` — so `_hydrate_chunks` always returned 0 and the LLM only ever saw bare entity labels with no prose. It correctly refused (`"I cannot find this in the knowledge base."`) on questions whose top entities were chunk-backed. Fix: build the chunk without `id`, assign `chunk.id` post-construction; bumped the two swallow-on-parse-fail `logger.debug` calls to `logger.warning` so a future regression like this surfaces. Lesson: a broad `except` that downgrades to `DEBUG` can hide a total feature outage — reserve that pattern for genuinely-skippable rows, not "should never fail" parsing.
- **Persisted turns carry a retrieval snapshot so reopened sessions match live asks (2026-05-11).** The `/chat` page builds `UITurn`s from `GET /qa/sessions/{id}` when you reopen a session; previously those persisted `ConversationTurn`s only stored `cited_*_ids`, so `turnToUITurn` synthesised an empty `RetrievalTrace` and no source cards — a reopened chat looked broken compared to a live one. Now `QAService._append_turn` stashes a compact JSON-safe snapshot on `turn.metadata.retrieval_snapshot` (`sources` = `RetrievedItem.to_dict()` minus the nested `metadata` sub-dict, `chunk_preview` truncated to 600 chars, capped at 24 rows; plus `cited_source_indices` and the full `retrieval_trace.to_dict()`). `ConversationTurn.to_dict()` already round-trips `metadata` through Neo4j, so no schema or API change was needed — `GET /qa/sessions/{id}` surfaces it verbatim and the frontend's `turnToUITurn` reconstructs a real `AskResponse` from it (falling back to the empty stub for turns that predate the snapshot). `_append_turn` now also stamps `turn.latency_ms` (was always 0). There's also a separate frontend fix: the chat page's session-load `useEffect` re-ran on every `sending` transition and clobbered the just-received live response with the empty stub — it's now gated on a `sidebarPickRef` so it only reloads turns when the user explicitly clicks a session in the sidebar.
...
2. ~~**LLM cost ceiling**~~ — **resolved 2026-05-10**. QA-flow LLM defaults to `gpt-4o-mini` (env `QA_LLM_MODEL_NAME`, settable per-request via `AskRequest.model`); ingestion stays on `gpt-4o` (env `LLM_MODEL_NAME`) since extraction quality matters more there. The per-call override threads through `LLMService.generate_text` / `generate_text_stream` / `generate_with_tools` as an optional `model` kwarg, with `**({"model": model} if model else {})` splat to stay backwards-compatible with fake LLMs in tests. Tests in `test_qa_service.py::test_ask_*_model_*`.
3. ~~**Cross-encoder model**~~ — **resolved 2026-05-10**. Keep `cross-encoder/ms-marco-MiniLM-L-6-v2` (already shipped). A biomedical CE is a follow-up ablation, not a day-one swap — the eval harness exists and we'd rather measure than guess. Revisit if the §9 numbers stall on a recall ceiling that looks domain-vocab-bound.
...
5. ~~**Gold set sourcing**~~ — **resolved 2026-05-10**. Mine the curation review queue. `scripts/seed_gold_from_curation.py` reads approved entities + relationships (rows with `metadata.annotations.verification_status = "approved"`), emits a YAML draft matching `tests/eval/rag_gold.yaml`'s schema with auto-generated questions ("What is {name}?" / "How does {source} {rel_type} {target}?") and the row's `source_chunk_ids` as `gold_chunk_ids`. SME tightens phrasing + adds `gold_answer_substrings` before merging. Run as `python scripts/seed_gold_from_curation.py --limit 50 --out tests/eval/rag_gold_draft.yaml`.
...
7. ~~**Hard delete**~~ — **resolved 2026-05-10**. Soft-delete only in v1. Hard delete would break the audit + undo story (§7.4, §7.5) and the curator's ability to revert a bad merge. Revisit if/when a real compliance request requires irreversible removal.
8. ~~**Audit retention**~~ — **resolved 2026-05-10**. No retention policy in v1 — chat-originated and curator-originated audit rows both persist indefinitely. Disk growth is a follow-up problem; current ingest scale doesn't justify the operational overhead of a scheduled-deletion job + the consistency-with-undo edge cases it introduces. Revisit if (a) chat traffic ramps significantly or (b) compliance imposes a retention ceiling.
